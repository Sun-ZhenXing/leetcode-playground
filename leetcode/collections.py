...
from typing import Any, List, Iterable, Iterator, Optional, Union, get_args, get_origin
...
class ListNode:
    ...
    def __repr__(self) -> str:
        ...
        if ring is not None:
            raise ValueError("linked list has ring at " "'{}'".format(ring.val))
    ...
    @staticmethod
    def exist_ring(node: "ListNode") -> "ListNode | None":
        """
        检查链表内是否包含环，
...
class Node:
    ...
    def __init__(
        self, val: int | None = None, children: Iterable["Node"] | None = None
    ):
        self.val = val
...
def make_tree(__iterable: Iterable[int | None]) -> "Node":
    """
    通过层序遍历生成 N 叉树
...
def make_binary_tree(__iterable: Iterable[int | None]) -> TreeNode | None:
    """
    通过层序遍历生成二叉树
    ...
    head = TreeNode(arr[0])  # type: ignore
    queue.append(head)
    i = 1
    while queue:
        cur = queue.popleft()
...
def make_linked_list(__iterable: Iterable[int]) -> "ListNode | None":
    """
    通过列表生成链表，不包含头结点
    @param arr 列表
    """
...
def debug(func: Union[MethodType, FunctionType, type]) -> Any:
    """
    ...
    # 如果是类的实例
    elif type(func) is not MethodType and type(func) is not FunctionType:
        cls_ = type(func)
        instance = func
    else:
        cls_ = None
    ...
    if cls_ is not None:
        method_list = [
            name for name in dir(cls_) if inspect.isfunction(getattr(cls_, name))
        ]
    ...
    for param in params:
        ...
        if annotation is inspect._empty:
            annotation = Any
        # 如果是容器类型，则是 Any
        if get_origin(annotation) in {dict, list, tuple, set, frozenset}:
            annotation = Any
        ...
        if expr == "":
            expr = "None"
        param_list.append(any_praser(expr, annotation))  # type: ignore
    return func(*param_list)


def print_debug(func: Union[MethodType, FunctionType, type]) -> None:
    """
